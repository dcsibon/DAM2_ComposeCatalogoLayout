...
fun SolActividad2() {
    var showLoading by rememberSaveable { mutableStateOf(false) }
    var txtButton by rememberSaveable { mutableStateOf("Cargar perfil") }
    var sizeDp by rememberSaveable { mutableIntStateOf(0) }
...
fun SolActividad3() {
    ...
    Column(
        ...
        Row(
            ...
            Button(
                onClick = { if (progressStatus >= 0.1f) progressStatus -= 0.1f }
            ) {
                Text(text = "Reducir")
            }
        }
 ...
 * State hoisting: elevación de un estado a una función superior.
 */
@Composable
fun SolPrevActividad5() {
